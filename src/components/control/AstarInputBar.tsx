// src/components/control/AstarInputBar.tsx
...
import { MIN_GRID_SIZE, MAX_GRID_SIZE } from "../../lib/astar-grid-utils";
...
export default function AstarInputBar() {
  const toggleAstarInput = useLayoutStore((s) => s.toggleAstarInput);

  const array = useAstarGridStore((s) => s.array);
  const rows = useAstarGridStore((s) => s.rows);
  const cols = useAstarGridStore((s) => s.cols);
  const wallDensity = useAstarGridStore((s) => s.wallDensity);
  const setRows = useAstarGridStore((s) => s.setRows);
  const setCols = useAstarGridStore((s) => s.setCols);
  const setStart = useAstarGridStore((s) => s.setStart);
  const setGoal = useAstarGridStore((s) => s.setGoal);
  const setWallDensity = useAstarGridStore((s) => s.setWallDensity);
  const randomize = useAstarGridStore((s) => s.randomize);
  const reset = useAstarGridStore((s) => s.reset);

  const startRow = array[2] ?? 1;
  const startCol = array[3] ?? 1;
  const goalRow = array[4] ?? rows - 2;
  const goalCol = array[5] ?? cols - 2;
  ...
  return (
    <div className="flex items-center gap-1.5 px-1 overflow-x-auto min-w-0">
      ...
      {/* Start position */}
      <span className="text-xs text-tn-muted whitespace-nowrap">Start</span>
      <PortalSelect
      ...
      {/* Goal position */}
      <span className="text-xs text-tn-muted whitespace-nowrap">Goal</span>
      <PortalSelect
        value={String(goalRow)}
        options={rowOptions}
        onChange={(v) => setGoal(Number(v), goalCol)}
      />
      <PortalSelect
        value={String(goalCol)}
        options={colOptions}
        onChange={(v) => setGoal(goalRow, Number(v))}
      />
      ...
      <IconBtn onClick={toggleAstarInput} title="Collapse input" className="w-7 h-7 ml-auto">
