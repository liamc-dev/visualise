...
function dfs(grid: number[][], sr: number, sc: number): number[][] {
  const rows = grid.length;
  const cols = grid[0].length;

  [[dfs.init.stack]]const stack: [number, number][] = [];[[/dfs.init.stack]]
  [[dfs.init.visited]]const visited: boolean[][] = Array.from(
    { length: rows }, () => Array(cols).fill(false)
  );[[/dfs.init.visited]]
  [[dfs.init.order]]const order: number[][] = Array.from(
...
export const DFS_TS_POINTER_HINTS = {
  "dfs.pop": ["cur"],
  "dfs.check": ["cur"],
  "dfs.oob": ["cur"],
  "dfs.wall": ["cur", "nb"],
...
export const DFS_TS_POINTER_LABELS = {
  cur: "(r,c)",
  nb: "(nr,nc)",
} as const satisfies Record<string, string>;
