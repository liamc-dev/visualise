// src/generators/algorithms/pathfinding/prims/prims-layout.ts
// Scene builder + geometry helpers for Prim's MST visualization.
...
const KEY_Y = 12;
const KEY_LABEL_Y = 11.65;
const PARENT_Y = 14;
const PARENT_LABEL_Y = 13.65;
...
export type BuildSceneOpts = {
  ...
  keyToneOverrides?: Record<string, TraceTone>;
  keyWeightOverrides?: Record<string, 0 | 1 | 2 | 3>;
...
export function buildScene(
  ...
  const { graph, labels, key, parent, inMST, mstEdges, curveVias } = st;

  // --- Graph nodes ---
  const nodes: TraceNode[] = graph.nodes.map((gn) => {
    const k = key[gn.label];
    const isInMST = inMST.has(gn.label);

    const tone: TraceTone = isInMST
      ? "accent"
      : k < Infinity ? "info" : "muted";
  ...
  // --- Key table ---
  const keyCaption: TraceOverlay = {
    kind: "caption", id: "pr:key-label",
    x: -1, y: KEY_Y, text: "key[]", emphasis: "soft",
  };
  const keyLabelOverlays: TraceOverlay[] = labels.map((l, i) => ({
    kind: "caption" as const, id: `pr:klbl:${l}`,
    x: TABLE_X_POSITIONS[i], y: KEY_LABEL_Y, text: l,
  }));
  const keyNodes: TraceNode[] = labels.map((l, i) => {
    const k = key[l];
    const toneOvr = opts?.keyToneOverrides?.[l];
    const wOvr = opts?.keyWeightOverrides?.[l];
    const tone: TraceTone = toneOvr ?? (k < Infinity ? "info" : "muted");
    return {
      id: `pr:k:${l}`, kind: "cell",
      pos: { x: TABLE_X_POSITIONS[i], y: KEY_Y },
      meta: {
        value: k === Infinity ? "\u221e" : k, tone,
        ...(tone === "muted" ? { opacityMul: 0.5 } : undefined),
        ...(wOvr != null ? { weight: wOvr } : undefined),
      },
    };
  });
  ...
  const parentNodes: TraceNode[] = labels.map((l, i) => {
    ...
    return {
      ...
      meta: {
        value: hasPrev ? p : "\u2013",
        tone: pTone ?? (hasPrev ? "info" : "muted") as TraceTone,
  ...
  return {
    nodes: [...nodes, ...keyNodes, ...parentNodes],
    edges,
    overlays: [
      ...labelOverlays, ...weightOverlays,
      keyCaption, ...keyLabelOverlays,
