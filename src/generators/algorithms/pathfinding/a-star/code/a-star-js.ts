...
function aStar(grid, sr, sc, gr, gc) {
  ...
  [[as.init.g]]const g = Array.from({ length: rows },[[/as.init.g]]
    [[as.init.g]]() => Array(cols).fill(Infinity));[[/as.init.g]]
  [[as.init.f]]const f = Array.from({ length: rows },[[/as.init.f]]
    [[as.init.f]]() => Array(cols).fill(Infinity));[[/as.init.f]]
  [[as.init.visited]]const visited = Array.from({ length: rows },[[/as.init.visited]]
...
export const ASTAR_JS_POINTER_HINTS = {
  "as.loop": ["u"],
  "as.pick": ["u"],
  "as.goal": ["u"],
  "as.neighbors": ["u", "v"],
...
export const ASTAR_JS_POINTER_LABELS = {
  u: "(r,c)",
  v: "(nr,nc)",
} as const satisfies Record<string, string>;
