export const LOGREG_JAVA =
` // Logistic Regression via Gradient Descent
 // Minimizes BCE: L = -(1/n) Σ[yᵢ·log(ŷᵢ) + (1-yᵢ)·log(1-ŷᵢ)]

 public class LogisticRegression {
   [[log.data]]static double[] fit(double[] X1, double[] X2, double[] Y,
       double lr, int epochs) {[[/log.data]]
     [[log.init]]int n = X1.length;
     double w1 = 0, w2 = 0, b = 0;[[/log.init]]

     [[log.epoch]]for (int epoch = 0; epoch < epochs; epoch++) {[[/log.epoch]]

       [[log.predict]]double[] yHat = new double[n];
       for (int i = 0; i < n; i++) {
         [[log.predict.step]]double z = w1 * X1[i] + w2 * X2[i] + b;
         yHat[i] = 1.0 / (1.0 + Math.exp(-z));[[/log.predict.step]]
       }[[/log.predict]]
     ...
     [[log.done]]return new double[]{w1, w2, b};[[/log.done]]
   }
 }
`;

export const LOGREG_JAVA_POINTER_HINTS = {} as const satisfies Record<string, string[]>;

export const LOGREG_JAVA_POINTER_LABELS = {} as const satisfies Record<string, string>;
