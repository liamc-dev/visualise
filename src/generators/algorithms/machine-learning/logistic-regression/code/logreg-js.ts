...
` // Logistic Regression via Gradient Descent
 ...
 [[log.data]]function fit(X1, X2, Y, lr, epochs) {[[/log.data]]
   ...
   [[log.epoch]]for (let epoch = 0; epoch < epochs; epoch++) {[[/log.epoch]]
     ...
     [[log.predict]]const yHat = new Array(n);
     for (let i = 0; i < n; i++) {
...
export const LOGREG_JS_POINTER_HINTS = {} as const satisfies Record<string, string[]>;

export const LOGREG_JS_POINTER_LABELS = {} as const satisfies Record<string, string>;
