...
` #include <vector>
 ...
 [[reg.data]]static pair<double,double> fit(
   ...
   [[reg.epoch]]for (int epoch = 0; epoch < epochs; epoch++) {[[/reg.epoch]]
     ...
     for (int i = 0; i < n; i++) {
       [[reg.loss.step]]loss += pow(Y[i] - yHat[i], 2);[[/reg.loss.step]]
     }
...
export const LINREG_CPP_POINTER_HINTS = {} as const satisfies Record<string, string[]>;

export const LINREG_CPP_POINTER_LABELS = {} as const satisfies Record<string, string>;
