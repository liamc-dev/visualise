...
` // K-Means Clustering
 ...
 [[km.data]]function kmeans(points: Pt[], k: number, maxIter = 15) {[[/km.data]]
   const n = points.length;
   [[km.init]]const pick = new Set<number>();
   while (pick.size < k) pick.add(Math.floor(Math.random() * n));
   const centroids: Pt[] = [...pick].map(i => [...points[i]]);[[/km.init]]
...
export const KMEANS_TS_POINTER_HINTS = {} as const satisfies Record<string, string[]>;

export const KMEANS_TS_POINTER_LABELS = {} as const satisfies Record<string, string>;
